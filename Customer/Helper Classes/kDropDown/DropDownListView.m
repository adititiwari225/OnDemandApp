    ...
    if (self = [super initWithFrame:rect])
    {
      ...
      //  self.arryData=[[NSMutableArray alloc]init];
        ...
        if (isMultipleSelection) {
            ...
            [btnDone setFrame:CGRectMake(rect.origin.x+180,rect.origin.y-60, 82, 31)];
...
//    [dataDic setObject:userName forKey:@"user_name"];
//    [dataDic setObject:userIdStr forKey:@"user_id"];
    [cell addSubview:imgarrow];
    
    
    /*  Reward Popup
    cell.textLabel.text = [[_kDropDownOption objectAtIndex:row] objectForKey:@"Amount"] ;
    
    
    cell.textLabel.text =[NSString stringWithFormat:@"$%.2f",[[[_kDropDownOption objectAtIndex:row]objectForKey:@"Amount"] floatValue]];
    ...
    */
    ...
    cell.textLabel.text =[NSString stringWithFormat:@"%@",[_kDropDownOption objectAtIndex:row]];
    
    /*
    if ([sharedInstance.frinedPopUpCondition_str isEqualToString:@"Friend"]) {
    ...
    */
    ...
    //cell.textLabel.text = [[_kDropDownOption objectAtIndex:row] objectForKey:@"user_name"] ;
    return cell;
}
