...
- (UITableView *)initWithTextField:(UITextField *)textField inViewController:(UIViewController *) parentViewController withOptions:(NSDictionary *)options
{
    ...
    CGRect frame;
 /*
    if ([IsHomeScreen isHomeScreen]) {
        if (IS_IPHONE4) {
            frame = CGRectMake(textField.frame.origin.x, textField.frame.origin.y+textField.frame.size.height, textField.frame.size.width, 250);
        }
         frame = CGRectMake(textField.frame.origin.x, 185, textField.frame.size.width, 250);
    }
    else{
         frame = CGRectMake(textField.frame.origin.x, textField.frame.origin.y+textField.frame.size.height+75, textField.frame.size.width, 250);
    }
    */
    
    sharedInstance = [SingletonClass sharedInstance];
    
    if([sharedInstance.selectedAddressAutoComplete isEqualToString: @"Yes"]) {
       
        //textField.frame =
        frame = CGRectMake(textField.frame.origin.x, 70,textField.frame.size.width+60, 400);
        
    } else {

    frame = CGRectMake(textField.frame.origin.x, 120, textField.frame.size.width, 250);
...
- (SPGooglePlacesAutocompletePlace *)placeAtIndexPath:(NSIndexPath *)indexPath {
    return self.suggestionOptions[indexPath.row];
}
...
- (UITableViewCell *)tableView:(UITableView *)tableView cellForRowAtIndexPath:(NSIndexPath *)indexPath
{
    ...
    //cell.textLabel.font = [UIFont fontWithName:KMediumFontStyle size:13.0];
...
- (void)tableView:(UITableView *)tableView didSelectRowAtIndexPath:(NSIndexPath *)indexPath
{
   ...
   // [self.textField setText:[self.suggestionOptions objectAtIndex:indexPath.row]];
    NSLog(@"%@",[self placeAtIndexPath:indexPath].name);
    if (_autoCompleteDelegate && [_autoCompleteDelegate respondsToSelector:@selector(autoCompletion:didSelectAutoCompleteSuggestionWithIndex:withString:)]) {
        [_autoCompleteDelegate autoCompletion:self didSelectAutoCompleteSuggestionWithIndex:indexPath.row withString:[self placeAtIndexPath:indexPath].name];
    }
