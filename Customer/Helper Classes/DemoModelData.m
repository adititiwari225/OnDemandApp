...
- (void)loadFakeMessages
{
    ...
    NSLog(@"sharedInstance.messagessDataMArray ==== %@",sharedInstance.messagessDataMArray);
    self.messages = [[NSMutableArray alloc]init];
    
    // if (!sharedInstance.messagessDataMArray || ![sharedInstance.messagessDataMArray count]){
    
    for (int i =0; i<[sharedInstance.messagessDataMArray count]; i++) {
        
        //        kJSQDemoAvatarIdSquires = [NSString stringWithFormat:@"%@",[[sharedInstance.messagessDataMArray objectAtIndex:i] objectForKey:@"sender_id"]];
        
        
        //  [[messagedataConversationArray objectAtIndex:i]objectForKey:@"conversations"]
        
        
        NSString *senderIdStr = [NSString stringWithFormat:@"%@",[[sharedInstance.messagessDataMArray objectAtIndex:i]objectForKey:@"SenderID"] ];
        
        NSString *recepient_IdStr = [NSString stringWithFormat:@"%@",[[sharedInstance.messagessDataMArray objectAtIndex:i]objectForKey:@"ReceiverID"]];
        NSLog(@"%@",recepient_IdStr);
        
        NSString *messageStr = [NSString stringWithFormat:@"%@",[[sharedInstance.messagessDataMArray objectAtIndex:i]objectForKey:@"MessageText"]];
        
        NSString *userNameStr = [NSString stringWithFormat:@"%@",[[sharedInstance.messagessDataMArray objectAtIndex:i]objectForKey:@"UserName"]];
        
        NSString *imageUrlStr = [NSString stringWithFormat:@"%@",[[sharedInstance.messagessDataMArray objectAtIndex:i]objectForKey:@"Url"]];
        NSLog(@"%@",imageUrlStr);
        
        NSString *messageTimeStr = [NSString stringWithFormat:@"%@",[[sharedInstance.messagessDataMArray objectAtIndex:i]objectForKey:@"postDate"]];
        NSDateFormatter *dateFormatter2 = [[NSDateFormatter alloc] init];
        // [dateFormatter2 setDateFormat: @"yyyy-MM-dd HH:mm a zzz"];
        [dateFormatter2 setDateFormat: @"yyyy-MM-dd'T'HH:mm:ss.SSS'Z"];
        NSDate *finalOldtime = [dateFormatter2 dateFromString:messageTimeStr];
        
        [self.messages addObject:[[JSQMessage alloc] initWithSenderId:senderIdStr
                                                    senderDisplayName:userNameStr
                                                                 date:finalOldtime
                                                                 text:messageStr]];
    }
}
...
- (NSDate *) convertUTCTimeToLocalTime:(NSString *)dateString WithFormate:(NSString *)formate{
    
    //formate = @"yyyy-MM-dd'T'HH:mm:ss"
    [self.dateFormattor setDateFormat:formate];
    NSTimeZone *gmtTimeZone = [NSTimeZone timeZoneWithAbbreviation:@"UTC"];
    //Log: gmtTimeZone - GMT (GMT) offset 0
    
    [self.dateFormattor setTimeZone:gmtTimeZone];
    NSDate *dateFromString = [self.dateFormattor dateFromString:dateString];
    ...
    [self.dateFormattor setLocale:[NSLocale currentLocale]];
    [self.dateFormattor  setTimeZone:sourceTimeZone];
