...
//        self.minimumValueChange = [sharedInstance.selectedStartAgeStr intValue];
//        self.maximumValue = [sharedInstance.selectedEndAgeStr intValue];
//        int min = (int)self.minimumValueChange;
//        int max = (int)self.maximumValue;
//
//        NSLog(@"Minimum value %d",min);
//        NSLog(@"Max Vlvalueaue %d",max);
//
////        self.minimumValueChange = 50.0;
////        self.maximumValue = 70.0;
////        
//        [self setMinValue:min maxValue:max];
//        [self setLeftValue:min rightValue:max];
//
//        self.leftValue = self.minimumValueChange;
//        self.rightValue = self.maximumValue;
//        [self setNeedsLayout];
//
//    }else if(sharedInstance.selectedStartWeightStr.length){
//        self.minimumValueChange = 0.0f;
//        self.maximumValue = 300.0f;
//        self.leftValue = self.minimumValueChange;
//        self.rightValue = self.maximumValue;
//
//    }
//    else {
//    self.minimumValueChange = 18.0f;
//    self.maximumValue = 99.0f;
//    
//    self.leftValue = self.minimumValueChange;
//    self.rightValue = self.maximumValue;
//    }
    if (sharedInstance.heightSliderValue ) {
        if ([sharedInstance.strUnityTypeValue isEqualToString:@"1"]) {
            self.maximumValue = 96.0;
            self.minimumValueChange = 48.0;
            self.leftValue = self.minimumValueChange;
            self.rightValue = self.maximumValue;
        }
        else {
            self.maximumValue =(96.0*2.54);
            self.minimumValueChange = (48.0*2.54);
            self.leftValue = self.minimumValueChange;
            self.rightValue = self.maximumValue;
                 }
    }
        ...
        if(sharedInstance.selectedStartWeightStr.length){
            self.minimumValueChange = [sharedInstance.selectedStartWeightStr intValue];
            self.maximumValue = [sharedInstance.selectedEndWeightStr intValue];
            int min = (int)self.minimumValueChange;
            int max = (int)self.maximumValue;
...
- (void)setMinValue:(CGFloat)minValue maxValue:(CGFloat)maxValue {
    ...
    self.minimumValueChange = minValue;
}
...
- (void)layoutSubviews
{
    ...
    CGFloat trackRange = self.maximumValue - self.minimumValueChange;

    CGFloat leftX = floorf((self.leftValue - self.minimumValueChange) / trackRange * leftAvailableWidth);
    if (isnan(leftX)) {
        leftX = 0.0;
    }

    CGFloat rightX = floorf((self.rightValue - self.minimumValueChange) / trackRange * rightAvailableWidth);
    if (isnan(rightX)) {
        rightX = 0.0;
    }
...
- (void)setMinimumValue:(CGFloat)minimumValue
{
    ...
    _minimumValueChange = minimumValue;
...
{
    if (maximumValue <= self.minimumValueChange) {
        maximumValue = self.minimumValueChange + self.minimumDistance;
    }
...
- (void)setLeftValue:(CGFloat)leftValue
{
    ...
    if (leftValue < self.minimumValueChange) {
        leftValue = self.minimumValueChange;
...
- (void)setRightValue:(CGFloat)rightValue
{
    ...
    if (rightValue < allowedValue) {
        if (self.pushable) {
            CGFloat leftSpace = self.leftValue - self.minimumValueChange;
...
{
    CGFloat distance = self.maximumValue - self.minimumValueChange;
...
{
    if (self.maximumValue - self.minimumValueChange < self.minimumDistance) {
        self.minimumDistance = 0.0f;
    }
}
